...
app.config['JWT_SECRET_KEY'] = 'jwt-secret-string'  # 生产环境中应使用环境变量
...
class Deck(db.Model):
    ...
    # 组合唯一约束：同一用户下卡组名称不能重复
    __table_args__ = (db.UniqueConstraint('user_id', 'name', name='_user_deck_name_uc'),)
    ...
    def to_dict(self):
        return {
            ...
            'forget_line': self.forget_line,
            'omega': self.omega,
            'max_delta': self.max_delta,
            'cards_count': self.count_cards(),
            'new_count': min(self.count_new_cards(), self.count_overtime_cards()) if self.count_cards() > 0 else 0,
            'overtime_count': self.count_overtime_cards(),
            'review_count': self.count_review_cards(),
            'remembered_count': self.count_remembered_cards(),
        }
...
def create_deck():
    try:
        ...
        deck_name = data['name'].strip()
        if not deck_name:
            return jsonify({'error': 'Deck name cannot be empty'}), 400
        
        # 检查当前用户下卡组名是否已存在
        existing_deck = Deck.query.filter_by(user_id=current_user_id, name=deck_name).first()
        if existing_deck:
            return jsonify({'error': 'Deck name already exists'}), 400
        
        # 创建新卡组，自动生成ID
        new_deck = Deck(name=deck_name, user_id=current_user_id)
        db.session.add(new_deck)
        db.session.flush()  # 获取自动生成的ID
        
        # 创建默认安排
        new_arrangement = Arrangement(deck_id=new_deck.id, count=10)
...
def edit_deck(deck_id: int):
    ...
    try:
        ...
        # 更新其他参数
        forget_line = data.get("forget_line")
...
def edit_card(card_id: int):
    ...
    try:
        ...
        # 验证卡片所属的卡组是否属于当前用户
        deck = Deck.query.filter_by(id=card.deck_id, user_id=current_user_id).first()
        if deck is None:
